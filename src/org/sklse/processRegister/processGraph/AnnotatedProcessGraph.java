...
/*Created by Zaiwen FENG at 11 NOV 2015
 * to evaluate the scalability of the prototype
 * In the APG, we do not consider resource node like PMR Graph*/
...
public class AnnotatedProcessGraph {
	
    public AnnotatedProcessGraph() {
        g = new SimpleDirectedGraph<GraphNode, DefaultEdge>(DefaultEdge.class);
        process = new APGProcessDTO();
    ...
    public APGProcessDTO representing() {
        return process;
    }

    public void representing(APGProcessDTO process) {
     ...
     *      Revised by Zaiwen in 12 NOV 2015 for evaluting prototype, adding number count for annotated process node
     * 
     */
    public Set<GraphNode> getAllProcessElementNodes() {
        ...
        for (GraphNode n : s) {
            if (n instanceof ProcessNode || n instanceof SplitDependencyNode ||
                    ...
                    n instanceof SequenceDependencyNode || n instanceof AnnotatedProcessNode || n instanceof EventNode) {
                ns.add(n);
            }
        }
        return ns;
    ...
    /*this function has been revised*/
    public Set<AnnotatedProcessNode> getAllAnnotatedProcessNodes() {
        Set<AnnotatedProcessNode> result = new HashSet<AnnotatedProcessNode>();
        for (GraphNode graphNode : g.vertexSet()) {
            if (graphNode instanceof AnnotatedProcessNode) {
                result.add((AnnotatedProcessNode) graphNode);
            }
        }
        return result;
    }
    ...
    */
    ...
    public static void main(String args[]) {
        AnnotatedProcessGraph pg = new AnnotatedProcessGraph();
	...
	public void assingID() throws Exception {
		...
		private void AssingConditionId() throws Exception{
			...
			for(AnnotatedProcessNode pd : this.getAllAnnotatedProcessNodes()){
