...
<script language="JavaScript">
	function resultcheckadmin() {
		if (formAdmin.check.value.length != 4) {
			alert("请输入4位验证码!");
			formAdmin.check.focus();
			return (false);
		} else if (formAdmin.username.value.length < 1) {
			alert("请输入用户名!");
			formAdmin.username.focus();
			return (false);
		} else if (!checkSpecial(formAdmin.username.value)) {
			alert("用户名不能包含特殊字符!");
			formAdmin.username.focus();
			return false;
		} else if (formAdmin.userpassword.value.length < 1) {
			alert("请输入密码!");
			formAdmin.userpassword.focus();
			return (false);
		} else if (!checkSpecial(formAdmin.userpassword.value)) {
			alert("密码不能包含特殊字符!");
			formAdmin.userpassword.focus();
			return false;
		} else {
...
<body>
	...
	<ul>
		...
		<li class="noneline"><a href="help/about.jsp" ><s:i18n name="bpep"><s:text name="about"></s:text></s:i18n></a></li>
		 <li class = "noneline"><a href="I18NAction.action?local=en_US&url=adminlogin">English</a>
		 <ul class="user">
		    <li><a href="I18NAction.action?local=zh_CN&url=adminlogin">中文</a></li>
		  </ul>
		 </li>
		...
		<div id="accordion" class="accordion">
			...
			<div id="showdetails" class="accordion">
				...
				<form name="formAdmin" method="post"
					action="logincheck/checkAdminuser.jsp"
					onsubmit="return resultcheckadmin();">
					<table align="center">
						<tr>
