...
<script language="JavaScript">
	function resultcheckisv() {
		if (formISV.check.value.length != 4) {
			alert("请输入4位验证码!");
			formISV.check.focus();
			return (false);
		} else if (formISV.username.value.length < 1) {
			alert("请输入用户名!");
			formISV.username.focus();
			return (false);
		} else if (!checkSpecial(formISV.username.value)) {
			alert("用户名不能包含特殊字符!");
			formISV.username.focus();
			return false;
		} else if (formISV.userpassword.value.length < 1) {
			alert("请输入密码!");
			formISV.userpassword.focus();
			return (false);
		} else if (!checkSpecial(formISV.userpassword.value)) {
			alert("密码不能包含特殊字符!");
			formISV.userpassword.focus();
			return false;
		} else {
...
<body>
	...
	<ul>
		...
		<li class="noneline"><a href="help/about.jsp" ><s:i18n name="bpep"><s:text name="about"></s:text></s:i18n></a></li>
		 <li class = "noneline"><a href="I18NAction.action?local=en_US&url=login1">English</a>
		 <ul class="user">
		    <li><a href="I18NAction.action?local=zh_CN&url=login1">中文</a></li>
		  </ul>
		 </li>
...
<s:text name="hello"></s:text>
	...
	<br/>
		<div id="accordion" class="accordion">
			<h3>Independent Software Vender</h3>
			<div id="showdetails" class="accordion">
				<br />
				<form name="formISV" method="post"
					action="logincheck/checkISVuser.jsp"
					onsubmit="return resultcheckisv();" style="margin: 10px auto;">
					<table align="center">
						<tr>
