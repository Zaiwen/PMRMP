...
<script language="JavaScript">
	function resultcheckpp() {
		if (formPP.check.value.length != 4) {
			alert("请输入4位验证码!");
			formPP.check.focus();
			return (false);
		} else if (formPP.username.value.length < 1) {
			alert("请输入用户名!");
			formPP.username.focus();
			return (false);
		} else if (!checkSpecial(formPP.username.value)) {
			alert("用户名不能包含特殊字符!");
			formPP.username.focus();
			return false;
		} else if (formPP.userpassword.value.length < 1) {
			alert("请输入密码!");
			formPP.userpassword.focus();
			return (false);
		} else if (!checkSpecial(formPP.userpassword.value)) {
			alert("密码不能包含特殊字符!");
			formPP.userpassword.focus();
			return false;
		} else {
...
<div id="top">
	<ul>
		...
		<li class="noneline"><a href="help/about.jsp" ><s:i18n name="bpep"><s:text name="about"></s:text></s:i18n></a></li>
		 <li class = "noneline"><a href="I18NAction.action?local=en_US&url=login2">English</a>
		 <ul class="user">
		    <li><a href="I18NAction.action?local=zh_CN&url=login2">中文</a></li>
		  </ul>
		 </li>
...
<s:text name="hello"></s:text>
	...
	<br/>
		<div id="accordion" class="accordion">
			<h3>Process Provider</h3>
			<div id="showdetails" class="accordion">
				<br />
				<form name="formPP" method="post"
					action="logincheck/checkPPuser.jsp"
					onsubmit="return resultcheckpp();">
					<table align="center">
						<tr>
