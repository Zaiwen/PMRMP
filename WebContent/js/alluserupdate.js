...
function SelectProcess(){
	$processSelect=$("#processSelect option:selected");
	$processuser=$processSelect.attr("processuser");
	$userSelect=$("#userSelect option:selected");
	if($processSelect.val()==""){
		$userSelect.val("");
		$userSelect.text("--请先选择基础流程--");
	}else{
		$userSelect.val($processuser);
		$userSelect.text($processuser);
		$userSelect.attr("selected","selected");
	}
}
function SelectProcess2(){
	$processSelect=$("#processSelect2 option:selected");
	$processuser=$processSelect.attr("processuser");
	$userSelect=$("#userSelect option:selected");
	if($processSelect.val()==""){
		$userSelect.val("");
